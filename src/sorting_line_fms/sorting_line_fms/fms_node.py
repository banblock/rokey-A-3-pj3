...
class FleetManagementSystem(Node):
    ...
    def __init__(self):
        ...
        # fleet_driver.py 전용 출력 채널 — FMS는 여기 구독은 안 한다(자기 상태는
        # /control/emergency_stop 하나로만 결정하고, 방송은 그 결정을 fleet_driver에
        # 전달하는 용도일 뿐). /fms/emergency_stop에 직접(ros2 topic pub 등) 값을
        # 보내도 fleet_driver는 반응하지만, FMS는 모르는 채로 남아 _dispatch_tick
        # 정지 상태가 어긋날 수 있다 — 정지/해제는 항상 /control/emergency_stop으로.
        ...
        # 이 토픽 하나로만 결정한다. True면 _emergency_stopped를 켜고
        # /fms/emergency_stop에 True를 방송해 fleet_driver를 멈추고, False면 반대로
        # 끄고 False를 방송해 재개시킨다(_on_emergency_stop 참고).
