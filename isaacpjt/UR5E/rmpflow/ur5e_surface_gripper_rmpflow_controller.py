...
class RMPFlowController(mg.MotionPolicyController):
    ...
    def _add_chassis_obstacle(
        ...
        # 2026-07-27: 로봇 여러 대(amr_1, amr_2, ...)를 동시에 쓸 때 이 경로가
        # 고정("/World/_rmpflow_chassis_obstacle")이면 나중에 생성된 로봇이
        # 먼저 생성된 로봇의 챗시 장애물 prim을 그대로 덮어써버려서, 두 로봇의
        # RMPflow 인스턴스가 서로 다른 로봇의(엉뚱한) 위치를 챗시 장애물로
        # 참조하게 된다 - chassis_prim_path("/World/{robot_id}/chassis_link")에서
        # robot_id를 그대로 따와 로봇별로 분리한다.
        _robot_id_segment = chassis_prim_path.strip("/").split("/")[1]
        obstacle_path = f"/World/_rmpflow_chassis_obstacle_{_robot_id_segment}"
