...
x_random = random.uniform(0.5, 1) * 0.4
y_random = random.uniform(0.5, 1) * 0.4
CUBE_INIT_POS = np.array([x_random, y_random, 0.0515 / 2.0])   # 큐브 초기 위치
GOAL_BLUE_POS      = np.array([0.55, -0.35, 0.0])            # 목표 위치
GOAL_GREEN_POS      = np.array([-0.55, 0.35, 0.0])            # 목표 위치
EE_OFFSET     = np.array([0.0, 0.0, 0.2])               # 접근 높이
color = random.randint(0, 1)
cube_color = np.array([0.0, 0.0, 0.0])
if color == 0:
    cube_color = np.array([0.0, 0.0, 1.0])
elif color == 1:
    cube_color = np.array([0.0, 1.0, 0.0])
...
class M0609Task(BaseTask):
    ...
    def _create_scene(self, scene):
        ...
        self._cube = scene.add(
            DynamicCuboid(
                prim_path="/World/target_blue_cube",
                name="target_blue_cube",
        ...
        scene.add(
            VisualCuboid(
                prim_path="/World/goal_blue_marker",
                name="goal_blue_marker",
                position=GOAL_BLUE_POS,
                scale=np.array([0.06, 0.06, 0.001]),
                color=np.array([0.0, 0.0, 1.0]),
            )
        )
        print(f"  [OK] goal @ {GOAL_BLUE_POS}")

        scene.add(
            VisualCuboid(
                prim_path="/World/goal_green_marker",
                name="goal_green_marker",
                position=GOAL_GREEN_POS,
                scale=np.array([0.06, 0.06, 0.001]),
                color=np.array([0.0, 1.0, 0.0]),
            )
        )
        print(f"  [OK] goal @ {GOAL_GREEN_POS}")
    ...
    def get_observations(self, data):
        ...
        goal_pos = 0
        if data == 1:
            goal_pos = GOAL_BLUE_POS
        elif data == 2:
            goal_pos = GOAL_GREEN_POS
        return {
            self._robot.name: {
    ...
    def pre_step(self, control_index, simulation_time):
        cube_blue_pos, _ = self._cube.get_world_pose()
        if not self._task_achieved and np.mean(np.abs(GOAL_BLUE_POS - cube_blue_pos)) < 0.02:
    ...
    def post_reset(self):
        ...
        x_random = random.uniform(0.5, 1.5) * 0.4
        y_random = random.uniform(0.5, 1.5) * 0.4
        new_cube_pos = np.array([x_random, y_random, 0.0515 / 2.0])   # 큐브 초기 위치
        color = random.randint(0, 1)
        cube_color = np.array([0.0, 0.0, 0.0])
        if color == 0:
            cube_color = np.array([0.0, 0.0, 1.0])
            ...
            cube_color = np.array([0.0, 1.0, 0.0])
        self._cube.set_world_pose(position=new_cube_pos)
        self._cube.get_applied_visual_material().set_color(cube_color)
        self._task_achieved = False
...
def main():
    ...
    def _on_color_id(msg):
        detected_color_id[0] = msg.data

    sub = node.create_subscription(Int32, '/color_id', _on_color_id, 10)

    TARGET_MAP = {
        1: GOAL_BLUE_POS,   # 1: Blue
        2: GOAL_GREEN_POS,  # 2: Green
    }
    ...
    print(f"  목표 위치    = {GOAL_BLUE_POS}, {GOAL_GREEN_POS}")

    # ── C-4. Controller 실행 루프 ─────────────────────────────
    color_detected = None
    target_updated = False
    selected_pick_pos = task.get_cube_position()  # 재시작 시 바뀐 실제 큐브 위치를 반영
    selected_place_pos = GOAL_BLUE_POS  # 감지 전 임시값, 색상 판별되면 교체됨
    task_done = False
    was_playing = False
    ...
    while simulation_app.is_running():
        ...
        # Play 시작 감지 → 큐브 재배치 반영해서 상태 리셋
        if is_playing and not was_playing:
            my_world.reset()
            initialize_robot(robot, my_world)
            controller.reset()
            selected_pick_pos = task.get_cube_position()
            selected_place_pos = GOAL_BLUE_POS
            target_updated = False
            task_done = False
            detected_color_id[0] = None
            print(f"[재시작] 새 큐브 위치 = {selected_pick_pos}")

        if is_playing and not task_done:
            current_event = controller.get_current_event()

            # [핵심 로직] Event 0(접근 이동)이 끝나고 Event 1(하강) 시작 전 색상 판별
            if not target_updated and detected_color_id[0] is not None:
                color_id = detected_color_id[0]
                place_pos = TARGET_MAP.get(color_id)
                if place_pos is not None:
                    selected_place_pos = place_pos
                    target_updated = True
                    print(f"탐지 완료! ID:{color_id}, 위치 업데이트 완료.")
                    if controller.is_paused():
                        controller.resume()

            # 픽 위치(Event 0) 도달 후, 색상 판별 전까지는 그 자리에서 정지
            if current_event >= 1 and not target_updated and not controller.is_paused():
                print("[대기] 픽 위치 도착, color_id 수신 대기 중...")
                controller.pause()

            actions = controller.forward(
                picking_position=selected_pick_pos,
                placing_position=selected_place_pos,
