...
STOP_TRIGGER_SHOE_DELAY_SEC: Final[float] = 20.0
...
class SimulationNode:
    ...
    def _stop_all_conveyors(self) -> None:
        ...
        self._convey_velocity_attr.Set(0.0)
        self._convey_01_velocity_attr.Set(0.0)
        self._convey_02_velocity_attr.Set(0.0)
    ...
    def _leave_emergency_stop(self, now: float) -> None:
        ...
        self._convey_velocity_attr.Set(1.0)
        self._convey_01_velocity_attr.Set(1.0)
        self._convey_02_velocity_attr.Set(1.0)
        ...
        self._stop_trigger_sequences.append(
            {
                ...
                "stopped": False,
                "restarted": False,
                "shoe_processed": False,
            }
        )

    def _activate_stop_trigger_shoe(self) -> None:
        if self._stop_trigger_first_run:
            return

        shoe_path = (
            "/World/shoes/sneaker_0001_red_240_ok_01"
        )
        shoe_prim = self._stage.GetPrimAtPath(shoe_path)

        if shoe_prim.IsValid():
            shoe_prim.SetActive(True)
        else:
            self._stage.OverridePrim(shoe_path).SetActive(True)

        self._stop_trigger_first_run = True

        print(
            "[simulation] StopTrigger 20초 지연 신발 활성화: "
            f"{shoe_path}"
        )
    ...
    def _update_stop_trigger_sequences(
        ...
        for sequence in self._stop_trigger_sequences:
            ...
            if (
                not bool(sequence["shoe_processed"])
                and elapsed >= STOP_TRIGGER_SHOE_DELAY_SEC
            ):
                self._activate_stop_trigger_shoe()
                sequence["shoe_processed"] = True
